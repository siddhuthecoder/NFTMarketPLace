...
contract NFTMarketPlace is ERC721URIStorage {
    ...
    function createToken(
        ...
        _tokenIds.add(_tokenIds.length());
        uint256 newTokenId = _tokenIds.length();
    ...
    function reSellToken(uint256 tokenId, uint256 price) public payable {
        ...
        _itemsSold.remove(tokenId); // Remove tokenId from itemsSold set
        _transfer(msg.sender, address(this), tokenId);
    }
    ...
    function createMarketItemSale(uint256 tokenId) public payable {
        ...
        _itemsSold.add(tokenId); // Add tokenId to itemsSold set
    ...
    function fetchMarketItem() public view returns (MarketItem[] memory) {
        uint256 itemCount = _tokenIds.length();
        uint256 unSoldItemCount = itemCount - _itemsSold.length();
    ...
    function fetchMyNFT() public view returns (MarketItem[] memory) {
        uint256 totalCount = _tokenIds.length();
        uint256 itemCount = 0;
        uint256 currentIndex = 0;
